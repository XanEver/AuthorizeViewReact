...
export const ShortLogoIcon = (props: HTMLAttributes<HTMLElement>) => <Icon component={ShortLogoTN} {...props} />

export const LogoIcon = (props: HTMLAttributes<HTMLElement>) => <Icon component={LogoTN} {...props} />
...
const Login: FC = ({ children }) => {
        ...
        <Row style={{ width: "600px", height: "fit-content",border:"1px solid #f0f0f0" }}>
          <Col style={{ display: "flex", width: "100%", justifyContent: "center", margin: 15 }}>
            <Space direction='vertical'>
              <H level={3} style={{ textAlign: "center", margin: 0 }}>Авторизация</H>
              <P type='danger' style={{ textAlign: "center", marginBottom: 15,display:"block" }}>Вы не вошли в учетную запись</P>
              {children}
            </Space>
          </Col>
        </Row>
  ...
  return (
    <React.Fragment>
      <LoginWrapper>
        <Spin spinning={search !== ""} tip={"Авторизация..."}>
          <LogoIcon style={{ position: "fixed", top: "5px", left: "5px" }} />
          {/* @ts-ignore */}
          <Tabs tabBarStyle={{ margin:0 }}>
            <Tab tab="Авторизация по умолчанию" key={1}>
              <AuthWrapper>
                <Col style={{ display: "flex", width: "100%", justifyContent: "center" }}>
                  {/* @ts-ignore */}
                  <Button type="primary" htmlType="button" onClick={() => document.location.href = `https://auth.corp.tatneft.ru/login?Url=${encodeURIComponent(document.location)}`}>
                    Авторизация
                  </Button>
                </Col>
              </AuthWrapper>
            </Tab>
            <Tab tab="Альтернативная авторизация" key={2}>
              <AuthWrapper>
                  <Form
                    name="basic"
